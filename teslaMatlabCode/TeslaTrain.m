function [ weights ] = TeslaTrain( inputSet, observations, order, historyInclusion )
...
if nargin == 4
    % We have historic addition
    maximumHistoryDistance = max( historyInclusion );
    historyLength = length( historyInclusion );
    inputSet( :, end + historyLength ) = 0;
    
    addedIndex = 1;
    for historyIndex = historyInclusion
        inputSet( ( maximumHistoryDistance + 1 ):end, numberOfInputTypes + addedIndex ) = observations( ( ( maximumHistoryDistance + 1 ):end ) - historyIndex );
        addedIndex = addedIndex + 1;
    end
    inputSet( 1:maximumHistoryDistance, : ) = [];
    observations( 1:maximumHistoryDistance, : ) = [];
    numberOfObservations = numberOfObservations - maximumHistoryDistance;
    if numberOfObservations < 0
        error('History size cannot be greater than the number of observations');
    end
    numberOfInputTypes = numberOfInputTypes + historyLength;
end    
